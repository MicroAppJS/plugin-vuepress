...
module.exports = function(cli, options) {
    cli
        .command('dev [targetDir]', 'start development server', { allowUnknownOptions: true })
    ...
    cli
        .command('build [targetDir]', 'build dir as static site', { allowUnknownOptions: true })
    ...
    cli
        .command('eject [targetDir]', 'copy the default theme into .vuepress/theme for customization.', { allowUnknownOptions: true })
        .option('--debug', 'eject in debug mode')
        .action((dir = '.') => {
            return wrapCommand(eject)(path.resolve(dir));
        });

    cli
        .command('info', 'Shows debugging information about the local environment', { allowUnknownOptions: true })
        .action(() => {
