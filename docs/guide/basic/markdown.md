...
所有的标题将会自动地应用 anchor 链接，anchor 的渲染可以通过 [`markdown.anchor`](../config/markdown.md#markdown-anchor) 来配置。
...
你可以自定义通过配置 [config.markdown.externalLinks](../config/markdown.md#markdown-externallinks) 来自定义外部链接的特性。
...
想了解更多，请移步 [Front Matter](../deep/frontmatter.md)。
...
目录（Table of Contents）的渲染可以通过  [`markdown.toc`](../config/markdown.md#markdown-toc) 选项来配置。
